...
from nn_dataflow.core import InputLayer, ConvLayer, FCLayer, \
        PoolingLayer, EltwiseLayer,G_convLayer
...
#The group conv specification have been obtained from the caffe prototxt in amir gholami's github repo.

def sqznxt_blk(Network,nifm,nofm,inp_h,strd,no_sqz,_prevs,gno):
    ...
    Network.add('{}_sqz1_a'.format(i), G_convLayer(nifm,int(redu*nifm),inp_h,1,strd= strd,no_g= gno),prevs=_prevs)
    Network.add('{}_sqz1_b'.format(i), G_convLayer(int(nifm*redu),int(nifm*redu*0.5),inp_h,1,no_g= 1))

    Network.add('{}_sep3_w'.format(i), G_convLayer(int(nifm*redu*0.5),int(nifm*redu),inp_h,[3,1],no_g= gno))
    Network.add('{}_sep3_h'.format(i), G_convLayer(int(nifm*redu),int(nifm*redu),inp_h,[1,3],no_g= gno))

    Network.add('{}_exp1'.format(i), G_convLayer(int(nifm*redu),nofm,inp_h,1,no_g= 1))
